      ...
      <?php foreach ($transactions as $transaction) : ?>
        <div class="bg-white shadow-sm border !border-gray-100 rounded-md px-4 py-3 space-y-2.5">
          <div class="flex items-center justify-between">
            <p><span class="font-medium">Kode Transaksi</span> : <?= $transaction['transaction_id'] ?></p>
            <p class="text-[15px]"><?= $transaction['created_at'] ?></p>
          </div>
          <div class="h-px w-full bg-slate-200"></div>
          <div class="flex items-end justify-between">
            <div class="flex items-center gap-6">
              <div class="p-2 bg-white rounded shadow-sm w-fit"><img src="<?= $transaction['cover']?>" alt="" width="60px"></div>
              <div class="text-sm">
                <p> <span class="font-medium">Penulis :</span> <?= $transaction['author']?></p>
            ...
            <a href="/transaction/<?= $transaction['transaction_id'] ?>" class="rounded-lg bg-white text-sky-700 border-[1.5px] border-sky-600 w-fit px-4 min-h-[2.25rem] md:min-h-[2.5rem] flex items-center justify-center text-sm">Lihat Detail</a>
          </div>
        </div>
      <?php endforeach ?>
    </section>
